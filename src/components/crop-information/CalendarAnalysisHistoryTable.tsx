...
export const CalendarAnalysisHistoryTable: React.FC<{ data: CropInformationAnalytics[] }> = ({ data }) => {
  ...
  return (
    <div>
      ...
      />
      {showResultSheet && (
        <CropCalendarSheet id={selectedResult?.id} onClose={() => setShowResultSheet(false)} />
      )}
    </div>
  );
};
